/*
 * fmx.h
 * Firmware main cross-module definitions and API
 * Author: Daniel H Sagarra
 *
 * This header provides public types, events, and API for the main firmware module.
 */

#ifndef FMX_H_
#define FMX_H_

#ifdef __cplusplus
extern "C" {
#endif

// --- Includes ---
#include "main.h"
#include "app_threadx.h"
#include "tx_port.h"
#include "tx_api.h"

// --- Public Defines: Events ---
/**
 * @brief Event codes for the system event queue.
 * Each event is a unique bit value (ULONG).
 */
typedef ULONG fmx_events_t;

#define FMX_EVENT_EMPTY             ((fmx_events_t)0)
#define FMX_EVENT_MENU_REFRESH      ((fmx_events_t)1)
#define FMX_EVENT_KEY_DOWN          ((fmx_events_t)2)
#define FMX_EVENT_KEY_UP            ((fmx_events_t)3)
#define FMX_EVENT_KEY_ESC           ((fmx_events_t)4)
#define FMX_EVENT_KEY_ENTER         ((fmx_events_t)5)
#define FMX_EVENT_KEY_DOWN_LONG     ((fmx_events_t)6)
#define FMX_EVENT_KEY_UP_LONG       ((fmx_events_t)7)
#define FMX_EVENT_KEY_ESC_LONG      ((fmx_events_t)8)
#define FMX_EVENT_KEY_ENTER_LONG    ((fmx_events_t)9)
#define FMX_EVENT_KEY_EXT_1         ((fmx_events_t)10)
#define FMX_EVENT_KEY_EXT_2         ((fmx_events_t)11)
#define FMX_EVENT_TIME_OUT          ((fmx_events_t)12)
#define FMX_EVENT_END               ((fmx_events_t)13)

// --- Public Defines: Thread/Stack ---
#define FMX_STACK_SIZE         (1024 * 16)
#define FMX_THREAD_PRIORITY_10 10
#define FMX_THRESHOLD_10       10
#define FMX_SLICE_0            0

// --- Typedefs and Enums ---
/**
 * @brief Low power control states.
 */
typedef enum {
    FMX_LOW_POWER_DISABLE = 0U,
    FMX_LOW_POWER_ENABLE
} FMX_LowPowerControl_t;

/**
 * @brief General status codes for FMX modules.
 */
typedef enum {
    FMX_STATUS_NULL = 0,
    FMX_STATUS_OK,
    FMX_STATUS_ERROR,
    FMX_STATUS_BUSY,
    FMX_STATUS_TIMEOUT,
    FMX_STATUS_INVALIDA_PARAM,
    FMX_STATUS_OUT_OF_RANGE
} fmx_status_t;

/**
 * @brief Flow rate state machine for caudal detection.
 */
typedef enum {
    FMX_RATE_OFF = 0,
    FMX_RATE_TO_ON,
    FMX_RATE_ON,
    FMX_RATE_TO_OFF
} fmx_rate_status_t;

// --- Extern Variables ---
extern const uint32_t FMX_DEBUG_UART_1_ENABLE;
extern TX_QUEUE fmx_deferred_cmd_queue;

// --- Public API ---
/**
 * @brief Get the current flow rate state.
 */
fmx_rate_status_t FMX_GetRateStatus(void);

/**
 * @brief Initialize the FMX module and its RTOS resources.
 * @param memory_ptr Pointer to RTOS memory pool.
 * @return RTOS status code.
 */
UINT FMX_Init(VOID *memory_ptr);

/**
 * @brief Turn on the LCD backlight and reset its timer.
 */
void FMX_LcdBackLightOn(void);

/**
 * @brief Ensure a menu refresh event is present in the event queue.
 */
void FMX_RefreshEventTrue(void);

/**
 * @brief Trigger the Bluetooth slave connection process.
 */
void FMX_Trigger_BluetoothSlave(void);

#ifdef __cplusplus
}
#endif

#endif /* FMX_H_ */

/*** end of file ***/