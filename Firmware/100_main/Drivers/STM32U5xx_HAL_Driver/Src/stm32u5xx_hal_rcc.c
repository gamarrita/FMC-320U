...
HAL_StatusTypeDef HAL_RCC_DeInit(void)
{
    ...
    CLEAR_BIT(RCC->CR,
            RCC_CR_MSIKON | RCC_CR_MSIPLLSEL | RCC_CR_MSIPLLFAST | RCC_CR_MSIKERON | RCC_CR_CSSON | RCC_CR_HSION | RCC_CR_HSIKERON | RCC_CR_PLL1ON | RCC_CR_PLL2ON | RCC_CR_PLL3ON | RCC_CR_HSI48ON | RCC_CR_HSEON | RCC_CR_SHSION);
...
HAL_StatusTypeDef HAL_RCC_OscConfig(const RCC_OscInitTypeDef *pRCC_OscInitStruct)
{
                        ...
                        && (pll_config == RCC_PLLSOURCE_MSI)))
        {
            ...
            else
            {
                ...
                if (pRCC_OscInitStruct->MSIClockRange > __HAL_RCC_GET_MSI_RANGE())
                {
                    ...
                    if (sysclk_source == RCC_SYSCLKSOURCE_STATUS_MSI)
                    {
                        if (RCC_SetFlashLatencyFromMSIRange(pRCC_OscInitStruct->MSIClockRange)
                                != HAL_OK)
                        {
                            return HAL_ERROR;
                        }
    ...
    /*------------------------------ HSI48 Configuration -----------------------*/
    if (((pRCC_OscInitStruct->OscillatorType) & RCC_OSCILLATORTYPE_HSI48)
        ...
        if (__HAL_RCC_GET_SYSCLK_SOURCE() != RCC_SYSCLKSOURCE_STATUS_PLLCLK)
        {
            if ((pRCC_OscInitStruct->PLL.PLLState) == RCC_PLL_ON)
            {
                ...
                __HAL_RCC_PLL_CONFIG(pRCC_OscInitStruct->PLL.PLLSource,
                        pRCC_OscInitStruct->PLL.PLLMBOOST, pRCC_OscInitStruct->PLL.PLLM,
                        pRCC_OscInitStruct->PLL.PLLN, pRCC_OscInitStruct->PLL.PLLP,
                        pRCC_OscInitStruct->PLL.PLLQ, pRCC_OscInitStruct->PLL.PLLR);
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_RCC_ClockConfig(const RCC_ClkInitTypeDef *const pRCC_ClkInitStruct,
        uint32_t FLatency)
{
    HAL_StatusTypeDef status;
    uint32_t tickstart;
...
uint32_t HAL_RCC_GetPCLK3Freq(void)
{
    /* Get HCLK source and Compute PCLK2 frequency ---------------------------*/
    return (HAL_RCC_GetHCLKFreq()
            >> APBPrescTable[(RCC->CFGR3 & RCC_CFGR3_PPRE3) >> RCC_CFGR3_PPRE3_Pos]);
}
/**
...
void HAL_RCC_GetOscConfig(RCC_OscInitTypeDef *pRCC_OscInitStruct)
{
    ...
    pRCC_OscInitStruct->HSICalibrationValue = (uint32_t) ((RCC->ICSCR3 & RCC_ICSCR3_HSITRIM)
            >> RCC_ICSCR3_HSITRIM_Pos);
    ...
    pRCC_OscInitStruct->LSEState =
            (regval & (RCC_BDCR_LSEON | RCC_BDCR_LSEBYP | RCC_BDCR_LSESYSEN));
    ...
    pRCC_OscInitStruct->PLL.PLLM = (uint32_t) (((reg1val & RCC_PLL1CFGR_PLL1M)
            >> RCC_PLL1CFGR_PLL1M_Pos) + 1U);
    pRCC_OscInitStruct->PLL.PLLN = (uint32_t) (((reg2val & RCC_PLL1DIVR_PLL1N)
            >> RCC_PLL1DIVR_PLL1N_Pos) + 1U);
    pRCC_OscInitStruct->PLL.PLLQ = (uint32_t) (((reg2val & RCC_PLL1DIVR_PLL1Q)
            >> RCC_PLL1DIVR_PLL1Q_Pos) + 1U);
    pRCC_OscInitStruct->PLL.PLLR = (uint32_t) (((reg2val & RCC_PLL1DIVR_PLL1R)
            >> RCC_PLL1DIVR_PLL1R_Pos) + 1U);
    pRCC_OscInitStruct->PLL.PLLP = (uint32_t) (((reg2val & RCC_PLL1DIVR_PLL1P)
