...
static __attribute__((noinline))  uint32_t DMA_List_CheckNodesBaseAddresses(
        DMA_NodeTypeDef const *const pNode1, DMA_NodeTypeDef const *const pNode2,
...
static uint32_t DMA_List_CheckNodesTypes(DMA_NodeTypeDef const *const pNode1,
        DMA_NodeTypeDef const *const pNode2, DMA_NodeTypeDef const *const pNode3);
static void DMA_List_GetCLLRNodeInfo(DMA_NodeTypeDef const *const pNode, uint32_t *const cllr_mask,
        uint32_t *const cllr_offset);
static uint32_t DMA_List_FindNode(DMA_QListTypeDef const *const pQList,
        DMA_NodeTypeDef const *const pNode, DMA_NodeInQInfoTypeDef *const NodeInfo);
...
static void DMA_List_ConvertNodeToDynamic(uint32_t ContextNodeAddr, uint32_t CurrentNodeAddr,
        uint32_t RegisterNumber);
static void DMA_List_ConvertNodeToStatic(uint32_t ContextNodeAddr, uint32_t CurrentNodeAddr,
        uint32_t RegisterNumber);
...
static void DMA_List_UpdateStaticQueueNodesCLLR(DMA_QListTypeDef const *const pQList,
        uint32_t operation);
static void DMA_List_FormatNode(DMA_NodeTypeDef *const pNode, uint32_t RegisterIdx,
        uint32_t RegisterNumber, uint32_t Format);
...
HAL_StatusTypeDef HAL_DMAEx_List_Start(DMA_HandleTypeDef *const hdma)
{
            ...
            || ((dma_state == HAL_DMA_STATE_BUSY) && (ccr_value != 0U)))
    {
        ...
        if (hdma->State == HAL_DMA_STATE_READY)
        {
            ...
            hdma->Instance->CLLR = ((uint32_t) hdma->LinkedListQueue->Head & DMA_CLLR_LA)
                    | cllr_mask;
        }
...
HAL_StatusTypeDef HAL_DMAEx_List_InsertNode(DMA_QListTypeDef *const pQList,
        DMA_NodeTypeDef *const pPrevNode, DMA_NodeTypeDef *const pNewNode)
{
    uint32_t cllr_mask;
    ...
    else
    {
        ...
        if (pPrevNode == NULL)
        {
            pNewNode->LinkRegisters[cllr_offset] = ((uint32_t) pQList->Head & DMA_CLLR_LA)
                    | cllr_mask;
            pQList->Head = pNewNode;
        }
        ...
        else
        {
            ...
            if (DMA_List_FindNode(pQList, pPrevNode, &node_info) == 0U)
            {
                ...
                if (node_info.currentnode_pos == pQList->NodeNumber)
                {
                    ...
                    {
                        pNewNode->LinkRegisters[cllr_offset] = ((uint32_t) pQList->FirstCircularNode
                                & DMA_CLLR_LA) | cllr_mask;
                    }

                    pPrevNode->LinkRegisters[cllr_offset] = ((uint32_t) pNewNode & DMA_CLLR_LA)
                            | cllr_mask;
                }
...
HAL_StatusTypeDef HAL_DMAEx_List_RemoveNode_Head(DMA_QListTypeDef *const pQList)
{
    ...
    else
    {
        ...
        pQList->Head->LinkRegisters[cllr_offset] = 0U;
        pQList->Head =
                ((DMA_NodeTypeDef*) (current_addr + ((uint32_t) pQList->Head & DMA_CLBAR_LBA)));
    }
...
HAL_StatusTypeDef HAL_DMAEx_List_ReplaceNode(DMA_QListTypeDef *const pQList,
        DMA_NodeTypeDef *const pOldNode, DMA_NodeTypeDef *const pNewNode)
{
    uint32_t cllr_mask;
    uint32_t cllr_offset;
    DMA_NodeInQInfoTypeDef node_info;
    ...
    if (DMA_List_FindNode(pQList, pOldNode, &node_info) == 0U)
    {
        ...
        else if (node_info.currentnode_pos == pQList->NodeNumber)
        {
            ...
            if (pQList->FirstCircularNode == ((DMA_NodeTypeDef*) (node_info.currentnode_addr)))
            {
                ...
                pNewNode->LinkRegisters[cllr_offset] = ((uint32_t) pNewNode & DMA_CLLR_LA)
                        | cllr_mask;
        ...
        DMA_NodeTypeDef *const pNewNode)
{
    ...
    {
        /* Link first circular node to new node */
        pNewNode->LinkRegisters[cllr_offset] = ((uint32_t) pQList->FirstCircularNode & DMA_CLLR_LA)
                | cllr_mask;
    }
    else
    {
...
HAL_StatusTypeDef HAL_DMAEx_List_InsertQ(DMA_QListTypeDef *const pSrcQList,
        DMA_NodeTypeDef const *const pPrevNode, DMA_QListTypeDef *const pDestQList)
{
    uint32_t cllr_mask;
    uint32_t cllr_offset;
        ...
        DMA_NodeTypeDef *const pNode)
{
    ...
    pNode->LinkRegisters[NODE_CTR1_DEFAULT_OFFSET] = pNodeConfig->Init.DestInc
            | pNodeConfig->Init.DestDataWidth | pNodeConfig->DataHandlingConfig.DataAlignment
  ...
  if (pNodeConfig->DestSecure == DMA_CHANNEL_DEST_SEC)
  {
    ...
    {
        /* Prepare DMA channel transfer register 2 (CTR2) value */
        pNode->LinkRegisters[NODE_CTR2_DEFAULT_OFFSET] |= pNodeConfig->TriggerConfig.TriggerMode
    ...
    if ((pNodeConfig->NodeType & DMA_CHANNEL_TYPE_2D_ADDR) == DMA_CHANNEL_TYPE_2D_ADDR)
    {
        ...
        pNode->LinkRegisters[NODE_CBR1_DEFAULT_OFFSET] |=
                (((pNodeConfig->RepeatBlockConfig.RepeatCount - 1U) << DMA_CBR1_BRC_Pos)
                        & DMA_CBR1_BRC);
    ...
    if ((pNodeConfig->NodeType & DMA_CHANNEL_TYPE_2D_ADDR) == DMA_CHANNEL_TYPE_2D_ADDR)
    {
        ...
        if (pNodeConfig->RepeatBlockConfig.DestAddrOffset < 0)
        {
            ...
            pNode->LinkRegisters[NODE_CTR3_DEFAULT_OFFSET] |= (((uint32_t) blockoffset
                    << DMA_CTR3_DAO_Pos) & DMA_CTR3_DAO);
        }
        else
        {
            pNode->LinkRegisters[NODE_CTR3_DEFAULT_OFFSET] |=
                    (((uint32_t) pNodeConfig->RepeatBlockConfig.DestAddrOffset << DMA_CTR3_DAO_Pos)
                            & DMA_CTR3_DAO);
        }
        ...
        if (pNodeConfig->RepeatBlockConfig.BlkSrcAddrOffset < 0)
        {
            ...
            pNode->LinkRegisters[NODE_CBR2_DEFAULT_OFFSET] = ((uint32_t) blockoffset
                    & DMA_CBR2_BRSAO);
        }
        else
        {
        ...
        if (pNodeConfig->RepeatBlockConfig.BlkDestAddrOffset < 0)
        {
            ...
            pNode->LinkRegisters[NODE_CBR2_DEFAULT_OFFSET] |= (((uint32_t) blockoffset
                    & DMA_CBR2_BRSAO) << DMA_CBR2_BRDAO_Pos);
        }
        else
        {
            pNode->LinkRegisters[NODE_CBR2_DEFAULT_OFFSET] |=
                    (((uint32_t) pNodeConfig->RepeatBlockConfig.BlkDestAddrOffset
                            << DMA_CBR2_BRDAO_Pos) & DMA_CBR2_BRDAO);
        }
        ...
        DMA_NodeTypeDef const *const pNode)
{
    ...
    pNodeConfig->Init.SrcDataWidth = pNode->LinkRegisters[NODE_CTR1_DEFAULT_OFFSET]
            & DMA_CTR1_SDW_LOG2;
    pNodeConfig->Init.DestDataWidth = pNode->LinkRegisters[NODE_CTR1_DEFAULT_OFFSET]
            & DMA_CTR1_DDW_LOG2;
    ...
    pNodeConfig->DataHandlingConfig.DataAlignment = pNode->LinkRegisters[NODE_CTR1_DEFAULT_OFFSET]
            & DMA_CTR1_PAM;
#if defined (__ARM_FEATURE_CMSE) && (__ARM_FEATURE_CMSE == 3U)
  ...
  else
  {
    ...
    pNodeConfig->TriggerConfig.TriggerMode = pNode->LinkRegisters[NODE_CTR2_DEFAULT_OFFSET]
            & DMA_CTR2_TRIGM;
    pNodeConfig->TriggerConfig.TriggerPolarity = pNode->LinkRegisters[NODE_CTR2_DEFAULT_OFFSET]
            & DMA_CTR2_TRIGPOL;
    pNodeConfig->TriggerConfig.TriggerSelection = (pNode->LinkRegisters[NODE_CTR2_DEFAULT_OFFSET] &
    DMA_CTR2_TRIGSEL) >> DMA_CTR2_TRIGSEL_Pos;
    pNodeConfig->Init.TransferEventMode = pNode->LinkRegisters[NODE_CTR2_DEFAULT_OFFSET]
            & DMA_CTR2_TCEM;
    ...
    if ((pNodeConfig->NodeType & DMA_CHANNEL_TYPE_2D_ADDR) == DMA_CHANNEL_TYPE_2D_ADDR)
    {
        pNodeConfig->RepeatBlockConfig.RepeatCount =
                ((pNode->LinkRegisters[NODE_CBR1_DEFAULT_OFFSET] & DMA_CBR1_BRC) >> DMA_CBR1_BRC_Pos)
                        + 1U;
    }
    else
    {
    ...
    if ((pNodeConfig->NodeType & DMA_CHANNEL_TYPE_2D_ADDR) == DMA_CHANNEL_TYPE_2D_ADDR)
    {
        ...
        offset = (uint16_t) ((pNode->LinkRegisters[NODE_CTR3_DEFAULT_OFFSET] & DMA_CTR3_DAO)
                >> DMA_CTR3_DAO_Pos);
        ...
        offset = (uint16_t) ((pNode->LinkRegisters[NODE_CBR2_DEFAULT_OFFSET] & DMA_CBR2_BRDAO)
                >> DMA_CBR2_BRDAO_Pos);
        ...
        DMA_NodeTypeDef const *const pNode, DMA_NodeInQInfoTypeDef *const NodeInfo)
{
    ...
    if (pNode == NULL)
    {
        ...
        while (node_idx < pQList->NodeNumber)
        {
            ...
            else
            {
                ...
                currentnode_address = ((DMA_NodeTypeDef*) (currentnode_address
                        + ((uint32_t) pQList->Head & DMA_CLBAR_LBA)))->LinkRegisters[cllr_offset]
                        & DMA_CLLR_LA;
            }
    ...
    {
        NodeInfo->nextnode_addr =
                (((DMA_NodeTypeDef*) NodeInfo->currentnode_addr)->LinkRegisters[cllr_offset] &
        ...
        DMA_NodeInQInfoTypeDef const *const NodeInfo)
{
    ...
    while (node_idx < pQList->NodeNumber)
    {
        ...
        /* Reset node */
        ((DMA_NodeTypeDef*) (previousnode_address + ((uint32_t) pQList->Head & DMA_CLBAR_LBA)))->LinkRegisters[cllr_offset] =
                0U;
        ...
        uint32_t LastNode_IsCircular)
{
    ...
    while (node_idx < pQList->NodeNumber)
    {
        ...
        if (node_idx == 0U)
        {
            ...
            current_cllr_offset = (((DMA_NodeTypeDef*) currentnode_addr)->NodeInfo & NODE_CLLR_IDX)
                    >> NODE_CLLR_IDX_POS;
        }
        /* Calculate nodes addresses */
        else
        {
            ...
            /* Get current node information */
            currentnode_addr =
                    (((DMA_NodeTypeDef*) (previousnode_addr))->LinkRegisters[previous_cllr_offset]
                            & DMA_CLLR_LA) + ((uint32_t) pQList->Head & DMA_CLBAR_LBA);
            current_cllr_offset = (((DMA_NodeTypeDef*) currentnode_addr)->NodeInfo & NODE_CLLR_IDX)
                    >> NODE_CLLR_IDX_POS;
...
{
    uint32_t currentnode_addr = (uint32_t) pQList->Head;
    uint32_t current_cllr_offset = ((uint32_t) pQList->Head->NodeInfo & NODE_CLLR_IDX)
            >> NODE_CLLR_IDX_POS;
