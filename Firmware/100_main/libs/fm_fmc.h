...
FM_FMC_FactorCalGet();
uint32_t
FM_FMC_FactorCalSet(ufp3_t factor_cal);
double
FM_FMC_FactorKCalc(ufp3_t factor_k, fm_fmc_vol_unit_t unit);
double
FM_FMC_FactorKGet();
uint32_t
FM_FMC_FactorKSet(ufp3_t factor_k);
double
FM_FMC_FactorRateCalc(double factor_k, fm_fmc_time_unit_t unit);
fmx_status_t
FM_FMC_FactorRateSet(double factor_rate);

fm_fmc_totalizer_t
FM_FMC_GetEnviroment(void);
void
FM_FMC_Init(sensors_list_t);

// Las siguientes funciones operan en totalizar.rate
void
FM_FMC_CaptureSet(uint16_t pulse, uint16_t time);
ufp3_t
FM_FMC_RateCalc();
void
FM_FMC_RateClear();
...
uint8_t
FM_FMC_TotalizerFpSelGet();
void
FM_FMC_TotalizerFpInc();
void
FM_FMC_TotalizerStrUnitGet(char **string, fm_fmc_vol_unit_t);
void
FM_FMC_TotalizerTimeUnitSel(fm_fmc_time_unit_t sel);
fm_fmc_time_unit_t
FM_FMC_TotalizerTimeUnitGet();
fmx_status_t
FM_FMC_TotalizerTimeUnitSet(fm_fmc_time_unit_t time_unit);
fm_fmc_vol_unit_t
FM_FMC_TotalizerVolUnitGet();
uint32_t
FM_FMC_TotalizerVolUnitSet(fm_fmc_vol_unit_t);
uint16_t
FM_FMC_TicketNumberGet();
...
void
FM_FMC_PulseAdd(uint32_t pulse_delta);
