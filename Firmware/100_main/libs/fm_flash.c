...
=
{ .reset_counter = 0,  // Numero de resets del microcontrolador.
        .reset_factory = 0,
// Numero de resets de fabrica.
        };// Conteo de resets.
...
flash_chip_info_t FM_FLASH_ChipInfoRead()
{
    ...
    FM_FLASH_Read(FLASH_CHIP_INFO_START, (uint8_t*) &chip_info,
    FM_FLASH_CHIP_INFO_SIZE);
    return chip_info;
}
 ...
 * @note	  La cantidad de bytes a escribir debe ser una calidad entera de bloques,
...
uint32_t FM_FLASH_Write(uint32_t memory_address, uint8_t *data, uint16_t data_length)
{
    ...
    /*
     * La longitud  de datos a escribir debería ser una cantidad enteras de bloques.
     * Si no lo es ajusto a que lo sea, recortando los datos que no completen un bloque.
     * Si data_length es menor que un bloque no se escribe nada.
     */
...
/*
 * @brief   Lee un valor en memoria flash. Cuando se quiere copiar el valor algo almacenado en flash
 *          a una variable en RAM no se pueden igualar, hay que usar esta funcion.
 * @param   memory_address, dirección de memoria flash que se quiere leer
 * 		  	data_ptr, puntero a RAM donde se guardara los datos leidos desde la flash.
 * 		  	data_length, cantidad de bytes a leer.
 * @retval	longitud  de datos leidos.
 *
 */
uint32_t FM_FLASH_Read(uint32_t memory_address, uint8_t *data_ptr, uint16_t data_length)
{
    for (int i = 0; i < data_length; i++)
    {
        *(data_ptr + i) = (*(uint8_t*) (memory_address + i));
    }
